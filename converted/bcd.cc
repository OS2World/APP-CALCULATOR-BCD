...
#else
inline void addpp(bcd *a, const bcd *b, unsigned) {
        unsigned char carry=0;
        for(unsigned o=N-1; o>0; o--) {
                unsigned char v=a->d[o];
                v+=b->d[o];
                v+=carry;
                if(v>100) {
                        carry=1;
...
"  mov [edi],bl         "\
  ...
  modify exact [eax ebx ecx edx];
#else
inline void subpp(bcd *a, const bcd *b, unsigned) {
        unsigned carry=0;
        for(unsigned o=N-1; o>0; o--) {
                int v=a->d[o];
                v-=b->d[o];
...
"  inc eax      "\
  ...
  value [eax];
#else
inline int iszero(const bcd *a, int prec) {
        for(unsigned o=1; o<prec; o++)
                if(a->d[o]) return 0;
        return 1;        
}
#endif
...
#else
inline void m10(bcd *a, unsigned) {
        unsigned o;
        unsigned char c=0;
        for(o=N-1; o>0; o--) {
                unsigned char nc=a->d[o]/10;
                a->d[o] = (a->d[o]%10)*10 + c;
                c=nc;
        }
}
...
"end:                   "\
  ...
  modify [eax ebx ecx edx esi];
#else
inline void d10(bcd *a, unsigned) {
        unsigned char c=0;
        for(unsigned o=1; o<N; o++) {
                unsigned char nc=a->d[o]%10;
                a->d[o] = c*10 + a->d[o]/10;
                c=nc;
        }
